...
{
     u8 i;
		RCC_APB1PeriphClockCmd(RCC_APB1Periph_BKP|RCC_APB1Periph_PWR,ENABLE);//使能BKP|PWR时钟
	
		PWR_BackupAccessCmd(ENABLE);//使能或者失能RTC和后备寄存器访问
	
		if(BKP_ReadBackupRegister(BKP_DR1)!=0xF0F0)//如果读出F0F0表明第一次进行RTC配置
		{
			BKP_DeInit();//清除侵侵入检测中断的待处理位

			RCC_RTCCLKConfig(RCC_RTCCLKSource_LSE);//设置RTC时钟（RTCCLK）
			
			RCC_LSEConfig(RCC_LSE_ON);//设置外部低速晶振（LSE）
			while(RCC_GetFlagStatus(RCC_FLAG_LSERDY)==RESET&&i<10)//检查指定的RCC标志位设置与否)
			{
				i++;
				delay_ms(20);
				if(i==10)return 0;
			...
			RTC_SetPrescaler(0x7FFF);//设置RTC预分频的值
      ...
      BKP_WriteBackupRegister(BKP_DR1, 0xF0F0);	//向指定的后备寄存器中写入用户程序数据
		 return 1;
		}
		else
		{		
